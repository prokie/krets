...
mod tests {
    ...
    fn test_low_pass_filter_ac_sweep() {
        ...
        fn vout(freq: f64) -> (f64, f64) {
            let omega = 2.0 * std::f64::consts::PI * freq;
            let r: f64 = 1000.0;
            let c: f64 = 1e-6;
            let x = omega * r * c;
            (1.0 / (1.0 + x.powi(2)), -x / (1.0 + x.powi(2)))
        }
        ...
        for result in solution {
            let solution = result.clone();
            let frequency = solution.get("frequency").unwrap().re;
            assert!((solution.get("V(out)").unwrap().re - vout(frequency).0).abs() < 1e-3);
